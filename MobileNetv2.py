...
class LineLoss(nn.Module):
    ...
    def __call__(self, line):
        ...
        similarity = (x_diff_start * x_diff_end + y_diff_start * y_diff_end) / (
            ...
            * torch.sqrt(torch.square(x_diff_end) + torch.square(y_diff_end) + 0.0000000000001)
        )
...
class MobileNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        print(out5.shape)
        return (
            torch.mean(self.AvgPool14_7(self.AvgPool28_14(out1)), dim=1)
            + torch.mean(self.AvgPool14_7(out2), dim=1)
            + torch.mean(self.AvgPool14_7(out3), dim=1)
            + torch.mean(self.AvgPool14_7(out4), dim=1)
            + torch.mean(out5, dim=1)
        )
